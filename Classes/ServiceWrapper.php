...
class ServiceWrapper {
	...
	public function __call($methodName, $arguments) {
		if (!$this->request instanceof \TYPO3\CMS\Soap\Request) {
			throw new \TYPO3\CMS\Extbase\Exception('No SOAP request set', 1297091911);
		}
	...
	protected function convertArrayArgument($argument, $methodName, $parameterName, $parameterType) {
		if (preg_match('/^array<(.+)>$/', $parameterType, $matches)) {
			...
			throw new \TYPO3\CMS\Extbase\Exception('Could not parse array type for parameter ' . $parameterName . ' from type "' . $parameterType . '"', 1297166416);
		}
	}
	 ...
	 * @return void
	 */
	protected function initializeCall(\TYPO3\CMS\Soap\Request $request) {}
	...
	public function headers($arguments) {
		$headers = \TYPO3\CMS\Extbase\Utility\ArrayUtility::convertObjectToArray($arguments);
	...
	protected function handleException($exception, $className, $methodName) {
		...
		if ($exception instanceof \TYPO3\CMS\Extbase\Security\Exception\AuthenticationRequiredException) {
			throw new \SoapFault('Client', 'Authentication required', NULL, 'Security_AuthenticationRequired');
		}
		if ($exception instanceof \TYPO3\CMS\Extbase\Security\Exception\AccessDeniedException) {
		...
		if ($expectedException) {
			...
			if (!$exception instanceof \TYPO3\CMS\Extbase\Exception) {
				$exception = new \TYPO3\CMS\Extbase\Exception($exception->getMessage(), $exception->getCode(), $exception);
			}
			$identifier = $exception->getCode();
			if ($this->settings['exposeExceptionInformation']) {
				$message = $exceptionClassName . ' (' . $exception->getCode() . '): ' . $exception->getMessage();
				$stackTrace = $exception->getTraceAsString();
				$details = $stackTrace;
			} else {
					// TODO Move gethostname to Environment
				$message = 'Internal server error. The error was not logged as ' . $identifier . ' on ' . gethostname() . '.';
				$details = $identifier;
			}
		...
		if (isset($methodTagsValues['throws'])) {
			if (is_array($methodTagsValues['throws'])) {
				foreach ($methodTagsValues['throws'] as $throwsDefinition) {
					list($throwsType,) = \TYPO3\CMS\Extbase\Utility\ArrayUtility::trimExplode(' ', $throwsDefinition, TRUE);
	...
	protected function convertStdClassToObject($argument, $className, $parameterName) {
		$source = \TYPO3\CMS\Extbase\Utility\ArrayUtility::convertObjectToArray($argument);
		...
		if ($target !== NULL) {
			...
			throw new \TYPO3\CMS\Soap\MappingException('Could not map argument ' . $parameterName . ' to type ' . $className, $this->propertyMapper->getMessages());
		}
	}
	...
	protected function getMethodReturnAnnotation($className, $methodName) {
		...
		if (isset($methodTagsValues['return']) && isset($methodTagsValues['return'][0])) {
			...
			throw new \TYPO3\CMS\Extbase\Exception('Could not get return value for ' . $className . '::' . $methodName, 1305130721);
		}
	}
