...
class RequestBuilder {
	 ...
	 * @return \TYPO3\CMS\Soap\Request The request object or FALSE if the service object name could not be resolved
	 */
	public function build(\TYPO3\CMS\Extbase\Http\Request $httpRequest) {
		$requestUri = $httpRequest->getUri();
		$baseUri = $httpRequest->getBaseUri();

		$servicePath = $this->servicePathForRequestUri($requestUri->getPath());
		...
		$request = \TYPO3\CMS\Soap\Request::create($requestUri, 'POST');
	...
	protected function serviceObjectNameForServicePath($servicePath) {
		if (isset($this->pathToObjectNameMapping[$servicePath])) {
			...
			list($vendor, $packageKey, $servicePath) = explode('/', $servicePath, 3);
			$servicePath = str_replace('/', '\\', $servicePath);
			$serviceObjectNameCandidate = sprintf('%s\%s\Service\Soap\%sService', implode('\\', explode('.', $vendor)), $packageKey, $servicePath);
		}
