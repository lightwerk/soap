...
class WsdlGenerator {
	...
	public function generateWsdl($className) {
		if (!preg_match('/Service$/', $className)) {
			throw new \TYPO3\CMS\Extbase\Exception('SOAP service class must end with "Service"', 1288984414);
		}
		if (!$this->objectManager->isRegistered($className)) {
			throw new \TYPO3\CMS\Extbase\Exception('SOAP service class "' . $className . '" is not known', 1297073339);
		}
	...
	protected function getServicePath($className) {
		...
		return $this->settings['endpointUriBasePath'] . strtolower($classParts);
	}
	...
	public function reflectOperations($className) {
		...
		if (!class_exists($className)) {
			throw new \TYPO3\CMS\Extbase\Exception('Class "' . $className . '" is not known', 1311091776);
		}
		$methodNames = get_class_methods($className);
		foreach ($methodNames as $methodName) {
			if (!$this->reflectionService->isMethodPublic($className, $methodName) || strpos($methodName, 'inject') === 0) {
				continue;
			}
			$methodReflection = new \TYPO3\CMS\Extbase\Reflection\MethodReflection($className, $methodName);
	...
	protected function buildRequestMessage($className, $methodName, &$complexTypes, &$typeMapping) {
		...
		foreach ($methodParameters as $parameterName => $methodParameter) {
			if ($methodParameter['optional']) {
				throw new \TYPO3\CMS\Extbase\Exception('Optional method arguments are not allowed for SOAP operations, ' . $className . '::' . $methodName, 1305039276);
			}
	...
	protected function getMethodReturnAnnotation($className, $methodName) {
		...
		if (isset($methodTagsValues['return']) && isset($methodTagsValues['return'][0])) {
			...
			throw new \TYPO3\CMS\Extbase\Exception('Could not get return value for ' . $className . '::' . $methodName, 1288984174);
		}
	}
	...
	protected function getOrCreateType($phpType, &$complexTypes, &$typeMapping) {
		...
		if (preg_match('/^array<(.+)>$/', $phpType, $matches)) {
			...
			$classReflection = new \TYPO3\CMS\Extbase\Reflection\ClassReflection($phpType);
			...
			foreach ($methodNames as $methodName) {
				if (strpos($methodName, 'get') === 0 && $this->reflectionService->isMethodPublic($phpType, $methodName)) {
					...
					if ($classReflection->hasProperty($propertyName)) {
						$propertyReflection = new \TYPO3\CMS\Extbase\Reflection\PropertyReflection($phpType, $propertyName);
			...
			throw new \TYPO3\CMS\Extbase\Exception('Type ' . $phpType . ' not supported', 1288979369);
		}
		return $typeMapping[$phpType];
	}
	...
	protected function isPropertyRequired($className, $propertyName) {
		if ($this->reflectionService->isPropertyTaggedWith($className, $propertyName, 'validate')) {
			$annotations = $this->reflectionService->getPropertyTagValues($className, $propertyName, 'validate');
	...
	protected function renderTemplate($templatePathAndFilename, array $contextVariables) {
		$absolutePath = \TYPO3\CMS\Core\Utility\GeneralUtility::getFileAbsFileName($templatePathAndFilename);
		$templateSource = file_get_contents($absolutePath);
		if ($templateSource === FALSE) {
			throw new \TYPO3\CMS\Fluid\Core\Exception('The template file "' . $templatePathAndFilename . '" could not be loaded.', 1225709595);
		}
	...
	protected function buildRenderingContext(array $contextVariables) {
		...
		$renderingContext->injectTemplateVariableContainer(new \TYPO3\CMS\Fluid\Core\ViewHelper\TemplateVariableContainer($contextVariables));
		$renderingContext->injectViewHelperVariableContainer(new \TYPO3\CMS\Fluid\Core\ViewHelper\ViewHelperVariableContainer());
